...
vector<int> solution(vector<int> answers) {
    ...
    for (int i = 0; i < answers.size(); i++) {
        if (answer[i] == pattern1[i % pattern1.size()]) matchCnt[0]++;  // size 5

        if (answer[i] == pattern2[i % pattern2.size()]) matchCnt[1]++;  // size 8

        if (answer[i] == pattern3[i % pattern3.size()]) matchCnt[2]++;  // size 10
    }
