...
int main()
{
    ...
    vector<int> v3(4, 3);   // 초기화
    vector<int> v4(v3); // v3를 복사해서 초기화한다.
    ...
    vec[2] = 10; // 인덱스2번 원소 변경 // 시간복잡도 : 0(1)
    for (int value : vec) {
        cout << value << ". ";
    }
    cout << endl;

    // 벡터 원소 삽입/삭제 // 시간복잡도 : 0(1)
    // 맨뒤 원소 삽입
    ...
    for (int target : targets) {
        // numbers에서 원소 탐색
        auto it = numbers.find(target); // 시간복잡도 : 0(1)
    ...
    studentScores["Alice"] = 95;    // 데이터 삽입(할당)
    studentScores["Bob"] = 88;
    studentScores["Charlie"] = 92;
    studentGrades["Marie"] = 3.8;
    ...
    // find() 메서드로 검색할 것
    auto result = studentScores.find("Charle");
