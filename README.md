...
- 정의
    - 데이터를 구성하는 구조. 데이터를 원하는 요구에 따라 처리할 떄 효율적으로 처리하기 위해서
...
- 정의
    ...
    - `데이터를 처리할 떄 문제를 해결하는 논리적인 방법과 순서`
    - 알고리즘 `Algorithm`
...
- 복잡도 - 시간과 메모리를 얼마나 소모하는지 효율성을 따지는 척도
    ...
    - 공간 복잡도 : 자료수를 n이 증가할 때 컴퓨터 메모리를 얼마만큼 사용하는지 판단. 임베디드/펌웨어에서는 중요 
...
3. 이진 검색 : [소스](./basic/algorithm02/app02/app02.c)
    - 데이터가 키값우로 이미 정렬(sort) 되어 있다는 가정하에서 시작
...
4. `복잡도(Complexity)`
    ...
    - 복잡도 표현 함수 : 0(영문자가 대문자 오, 빅오)
    - 선행 검색 시간 복잡도 : $0(n)$
    - 이전 검색 시간 복잡도 : $0(log n)$
    - 시간복잡도 일반적 사용
...
- 공간복잡도
    - $0(1)$ - 변수 몇 개
    - $0(n)$ - 1차원 배열, 재귀 깊이 n, 트리전체 저장 자료구조
    - $0(n^2)$ - 2차원 배열


5. bsearch() 표준라이브러리 이진검색 함수 존재
    - 두 값 비교를 위한 함수 포인터용 비교함수를 작성해야함
    - p 123 이후 참조
...
2. 기술
    - `기초프로그래밍`, `알고리즘`, 통신프로그램, 데이터 분석, 웹개발, 윈앱개발, IOT
...
3. `큐` : [소스](./basic/algorithm03/app01/app01.c)
    - 양쪽이 다 열려있어서 한쪽에서 데이터를 집어넣으면 반대에서 반대에서 데이터를 꺼내는 자료구조
    - 영화관 티켓줄, 버스대기줄, 식당대기줄
    - 컴퓨터 예, 파일입출력 스트림, 프린터 출력대기(pool), 게임키보드입력순서 버퍼
...
5. `메모이제이션!(memoization)` : [소스](./basic/algorithm04/app01/app01.c)
    ...
    - 이미 계산한 결과를 저장해두고 다시 호출하면 재계산하지 않고 저장된 결과를 바로 반환, 사용
    - 동적계획법에 많이 사용
    - 단점 - 메모리 매우 많이 사용. 프로그램이 비정상 종료 될 수. 동적할당으로 변경 필수 
...
1. 데이터를 기준값에 따라 오름차순(작은값부터 큰값으로), 내림차순(큰값으로 작은값으로)데이터 나열
...
2. KMP - 부루트-포스 방법에서 패턴 형태로 변형된 비교 방법

3. 보이어-무어 - 부르트-포스와 KMP 상에서 패턴으로 옮길 크기 정해서 검색방법

### 리스트 자료구조

1. 데이터를 순서대로 나열한 자료구조 - 배열(선형 리스트)과 동일
    - 배열 사용할 경우 중간에 데이터가 빠지거나 추가되면 전부 밀어내는 추가 처리 필요
...
1. 검색과 데이터 추가, 삭제가 효율적으로 사용할 수 있는 자료구조
    - 특징 키값으로 매핑할 수 있는 선형 자료구조
    - 해시 함수 - 임의 데이터를 고정된 길의 키값으로 변형해주는 함수
    - 자료 접근(검색)이 $0(1)$ 정도로 매우 빠름 실제 값(value)를 같이 저장
    - 해시 함수를 통해서 만들어진 key와 실제 값(value)를 같이 저장

2. 동일한 key가 만들어진 충돌 가능할 단점 존재
